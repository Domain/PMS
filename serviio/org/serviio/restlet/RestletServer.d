...
public class RestletServer
{
  ...
  private static void prepareComponent(Component component, Map<String, Application> applications, int port) {
    ...
    for (Entry<String, Application> application : applications.entrySet()) {
      if (ObjectValidator.isNotEmpty(remoteHost))
        log.info(String.format("Starting Restlet server (%s) exposed on %s:%s", new Object[] { application.getKey(), remoteHost, Integer.valueOf(port) }));
      else {
        log.info(String.format("Starting Restlet server (%s) exposed on port %s", new Object[] { application.getKey(), Integer.valueOf(port) }));
      }
