  ...
  : BufferWrapper
{
  ...
  public String readString()
    ...
    try
    {
      return out.toString("UTF-8"); } catch (UnsupportedEncodingException e) {
    }
  ...
  private int parse8(int i) {
    ...
    if (i > readBitsRemaining) {
      ...
      int buffer = (readBitsBuffer & (int)(Math.pow(2.0D, readBitsRemaining) - 1.0D)) << resultRemaining;

      readBitsBuffer = readByte();
      readBitsRemaining = (8 - resultRemaining);
      int movedAndMasked = readBitsBuffer >>> readBitsRemaining & (int)(Math.pow(2.0D, resultRemaining) - 1.0D);

      return buffer | movedAndMasked;
    }
    readBitsRemaining -= i;

    return readBitsBuffer >>> readBitsRemaining & (int)(Math.pow(2.0D, i) - 1.0D);
  }
