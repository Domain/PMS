    ...
    JpegUtils.Visitor visitor = new JpegUtils.Visitor() {
      ...
      private void parseSOFSegment(int markerLength, byte[] segmentData)
        ...
        if (toBeProcessed > 6) {
          ...
          numComponents = binaryParser.readByte("Number_of_components", is, "Unable to read Number of components from SOF marker");
        ...
        {
          binaryParser.skipBytes(is, 1, "Not a Valid JPEG File");
          imageParams.setSamplingMode(binaryParser.readByte("Sampling Factors", is, "Unable to read the sampling factor from the 'Y' channel component spec"));

          binaryParser.readByte("Quantization Table Index", is, "Unable to read Quantization table index of 'Y' channel");

          binaryParser.skipBytes(is, 2, "Not a Valid JPEG File");
          binaryParser.readByte("Quantization Table Index", is, "Unable to read Quantization table index of 'Cb' Channel");
        }
      }
    };
