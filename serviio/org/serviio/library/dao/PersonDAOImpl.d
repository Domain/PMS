  ...
  : PersonDAO
{
  ...
  {
    log.debug_(String.format("Reading a Person (name = %s)", new Object[] { name }));
    Connection con = null;
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot read Person with name = %s", new Object[] { name }), e);
    } finally {
  ...
  {
    log.debug_(String.format("Reading a Person (id = %s)", new Object[] { id }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read Person with id = %s", new Object[] { id }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public Long addPersonToMedia(String personName, RoleType role, Long mediaItemId)
  {
    ...
    log.debug_(String.format("Adding a Person %s to media item %s as %s", new Object[] { personName, mediaItemId, role.toString() }));
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot add Person with name %s to media item %s", new Object[] { personName, mediaItemId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public Long addPersonToMusicAlbum(String personName, RoleType role, Long albumId) {
    ...
    log.debug_(String.format("Adding a Person %s to album %s as %s", new Object[] { personName, albumId, role.toString() }));
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot add Person with name %s to music album %s", new Object[] { personName, albumId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public void removeAllPersonsFromMedia(Long mediaItemId)
  {
    ...
    log.debug_(String.format("Removing all Persons from media item %s", new Object[] { mediaItemId }));

    List<Person> persons = retrievePersonsForMediaItem(mediaItemId);

    log.debug_(String.format("Found all Persons (%s) for media item %s", new Object[] { Integer.valueOf(persons.size()), mediaItemId }));
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot remove all Persons from media item %s", new Object[] { mediaItemId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public void removeAllPersonsFromMusicAlbum(Long albumId)
  {
    ...
    log.debug_(String.format("Removing all Persons from album %s", new Object[] { albumId }));
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot remove all Persons from musicalbum %s", new Object[] { albumId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  {
    log.debug_(String.format("Retrieving list of persons with role %s (from=%s, count=%s)", new Object[] { roleType, Integer.valueOf(startingIndex), Integer.valueOf(requestedCount) }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving number of persons with role %s", new Object[] { roleType }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving number of roles for person %s", new Object[] { personId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read number of roles for person %s", new Object[] { personId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  {
    log.debug_(String.format("Retrieving list of persons with role %s for MediaItem %s", new Object[] { roleType, mediaItemId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving list of persons with role %s for MusicAlbum %s", new Object[] { roleType, albumId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving list of persons for MediaItem %s", new Object[] { mediaItemId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read list of persons for media item %s", new Object[] { mediaItemId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  {
    log.debug_(String.format("Retrieving list of persons for MusicAlbum %s", new Object[] { albumId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read list of persons for music album %s", new Object[] { albumId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public Long getPersonRoleForMediaItem(RoleType role, Long personId, Long mediaItemId)
  {
    ...
    log.debug_(String.format("Checking if person %s has a role %s for media item %s", new Object[] { personId, role.toString(), mediaItemId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot check if person %s has a role %s for media item %s", new Object[] { personId, role.toString(), mediaItemId }), e);
    }
    finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
  ...
  public Long getPersonRoleForMusicAlbum(RoleType role, Long personId, Long albumId)
  {
    ...
    log.debug_(String.format("Checking if person %s has a role %s for album %s", new Object[] { personId, role.toString(), albumId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot check if person %s has a role %s for music album %s", new Object[] { personId, role.toString(), albumId }), e);
    }
    finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
  ...
  public List<Long> getRoleIDsForMediaItem(RoleType role, Long mediaItemId)
  {
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot retrieve list or role IDs for role %s for media item %s", new Object[] { role.toString(), mediaItemId }), e);
    }
    finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
  ...
  {
    log.debug_(String.format("Retrieving list of person initials (role = %s, from=%s, count=%s)", new Object[] { role, Integer.valueOf(startingIndex), Integer.valueOf(requestedCount) }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving number of person initials for role %s", new Object[] { role }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving list of persons with initial %s and role %s (from=%s, count=%s)", new Object[] { initial, role, Integer.valueOf(startingIndex), Integer.valueOf(requestedCount) }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving number of persons with initial %s and role %s", new Object[] { initial, role }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  private long createPerson(Connection con, Person newInstance) {
    ...
    log.debug_(String.format("Creating a new Person (name = %s)", new Object[] { newInstance.getName() }));
    PreparedStatement ps = null;
    try {
      ...
      throw new PersistenceException(String.format("Cannot create Person with name %s", new Object[] { newInstance.getName() }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
    }
  }
  ...
  private void deletePerson(Connection con, Long personId) {
    log.debug_(String.format("Deleting a Person (id = %s)", new Object[] { personId }));
    PreparedStatement ps = null;
    try {
      ...
      throw new PersistenceException(String.format("Cannot delete Person with id = %s", new Object[] { personId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
    }
  }
