  ...
  : PlaylistDAO
{
  ...
  public long create(Playlist newInstance)
    ...
    log.debug_(String.format("Creating a new Playlist (title = %s)", new Object[] { newInstance.getTitle() }));
    Connection con = null;
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot create Playlist with name %s", new Object[] { newInstance.getTitle() }), e);
    } finally {
  ...
  public void delete(Long id)
    {
    log.debug_(String.format("Deleting a Playlist (id = %s) and all related items", new Object[] { id }));
    ...
    try {
      ...
      throw new PersistenceException(String.format("Cannot remove Playlist %s or its items", new Object[] { id }), e);
    } finally {
  ...
  public Playlist read(Long id) {
    log.debug_(String.format("Reading a Playlist (id = %s)", new Object[] { id }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read Playlist with id = %s", new Object[] { id }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public void update(Playlist transientObject) {
    ...
    log.debug_(String.format("Updating Playlist (id = %s)", new Object[] { transientObject.getId() }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot update Playlist with id %s", new Object[] { transientObject.getId() }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public bool isPlaylistPresent(File playlistFile)
  {
    ...
    log.debug_(String.format("Checking if DB already contains playlist %s", new Object[] { playlistFile.getName() }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      ResultSet rs = ps.executeQuery();
      if (rs.next()) {
        log.debug_(String.format("Playlist %s already exists in DB", new Object[] { playlistFile.getName() }));
        return true;
      }
      log.debug_(String.format("Playlist %s doesn't exist in DB yet", new Object[] { playlistFile.getName() }));
      return false;
    }
    catch (SQLException e) {
      throw new PersistenceException(String.format("Cannot check if playlist exists: %s ", new Object[] { playlistFile.getName() }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  {
    log.debug_(String.format("Reading Playlists for Repository (id = %s)", new Object[] { repositoryId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read Playlists for Repository with id = %s", new Object[] { repositoryId }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public void removeMediaItemFromPlaylists(final Long mediaItemId)
  {
    ...
    {
      throw new PersistenceException(String.format("Cannot delete MediaItem with id = %s from playlists", new Object[] { mediaItemId }), e);
    }
  }
  ...
  {
    log.debug_(String.format("Removing all items from playlist %s", new Object[] { playlistId }));
    try {
      new JdbcExecutor<Object>()
      {
        protected PreparedStatement processStatement(Connection con) {
    ...
    {
      throw new PersistenceException(String.format("Cannot delete PlaylistItems from playlist with id = %s", new Object[] { playlistId }), e);
    }
  }
  ...
  public void addPlaylistItem(Integer order, Long mediaItemId, Long playlistId)
  {
    ...
    log.debug_(String.format("Adding a new Item to playlist (media item = %s, playlist = %s)", new Object[] { mediaItemId, playlistId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Reading items for playlist %s", new Object[] { playlistId }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving list of Playlists for %s (from=%s, count=%s) [%s]", new Object[] { mediaType, Integer.valueOf(startingIndex), Integer.valueOf(requestedCount), accessGroup }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  {
    log.debug_(String.format("Retrieving number of playlists for %s [%s]", new Object[] { mediaType, accessGroup }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
  ...
  private void deletePlaylist(Connection con, final Long playlistId)
  {
    ...
    {
      throw new PersistenceException(String.format("Cannot delete Playlist with id = %s", new Object[] { playlistId }), e);
    }
  }
