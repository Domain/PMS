  ...
  : CoverImageDAO
{
  ...
  public long create(CoverImage newInstance)
    ...
    log.debug_(String.format("Creating a new ImageCover (length = %s)", new Object[] { Integer.valueOf(newInstance.getImageBytes().length) }));
    Connection con = null;
  ...
  public void delete(Long id) {
    log.debug_(String.format("Deleting a CoverImage (id = %s)", new Object[] { id }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot delete CoverImage with id = %s", new Object[] { id }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
  ...
  public CoverImage read(Long id) {
    log.debug_(String.format("Reading a CoverImage (id = %s)", new Object[] { id }));
    Connection con = null;
    PreparedStatement ps = null;
    try {
      con = DatabaseManager.getConnection();
      ...
      throw new PersistenceException(String.format("Cannot read CoverImage with id = %s", new Object[] { id }), e);
    } finally {
      JdbcUtils.closeStatement(ps);
      DatabaseManager.releaseConnection(con);
    }
