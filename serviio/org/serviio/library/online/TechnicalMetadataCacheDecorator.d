  ...
  : OnlineCacheDecorator<TechnicalMetadata>
{
  ...
  public void store(String url, TechnicalMetadata cachedValue)
  {
    try
    {
      ...
      log.debug_(String.format("Stored technical metadata for online item '%s' in the cache (%s), returning it", new Object[] { url, regionName }));
  ...
  public void evict(String url)
  {
    ...
    {
      cache.remove(url);
      log.debug_(String.format("Removed technical metadata fro online item '%s' from cache (%s)", new Object[] { url, regionName }));
    } catch (CacheException e) {
