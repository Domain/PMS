...
public class OnlineItemService
{
  ...
  private static final Logger log = LoggerFactory.getLogger(OnlineItemService.class_);
  ...
  public static OnlineResourceContainer<?, ?> findContainerResourceById(Long onlineRepositoryId)
  {
    ...
    if (resource !is null) {
      return (OnlineResourceContainer<?, ?>)resource.getOnlineItem();
    }
    return null;
  }
  ...
  public static NamedOnlineResource<OnlineResourceContainer<?, ?>> findNamedContainerResourceById(Long onlineRepositoryId)
  {
    ...
    try {
      ...
      if (resource !is null) {
        return new NamedOnlineResource<OnlineResourceContainer<?, ?>>(resource, onlineRepository.getRepositoryName());
      }
      return null;
    } catch (IOException e) {
  ...
  public static OnlineItem findOnlineItemById(Long onlineItemId)
    {
    NamedOnlineResource<?> namedItem = findNamedOnlineItemById(onlineItemId);
    if (namedItem !is null) {
      return (OnlineItem)namedItem.getOnlineItem();
    }
    return null;
  }
    ...
    for (OnlineRepository repo : allRepositories)
      try {
        ...
        if (parsedResource !is null)
          result.add(new NamedOnlineResource<OnlineResourceContainer<?, ?>>(parsedResource, repo.getRepositoryName()));
      }
      catch (IOException e)
      {
      }
