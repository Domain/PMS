...
public abstract class PluginExecutionProcessor<T>
{
  ...
  public T execute(int millisecondsTimeout)
    ...
    try
    {
      return (T) future.get(millisecondsTimeout, TimeUnit.MILLISECONDS);
      ...
      throw new RuntimeException(String.format("The operation took more than %s ms and has been cancelled.", new Object[] { Integer.valueOf(millisecondsTimeout) }));
    }
    finally {
      cancel(future);
    }
