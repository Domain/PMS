        ...
        while (subscrIt.hasNext()) {
          ...
          if (!subscription.getDuration().equals("infinite")) {
            try {
              ...
              {
                subscrIt.remove();
                log.debug_(String.format("Removed expired subscription %s from service %s", new Object[] { subscription.getUuid(), service.getServiceId() }));
              }
            }
            catch (NumberFormatException e) {
              log.warn(String.format("Provided subscription duration is not a number (%s), cancelling the subscription", new Object[] { subscription.getDuration() }));
  ...
  private void removeAllSubscriptions()
  {
    for (Service service : Device.getInstance().getServices()) {
      ...
      while (subscrIt.hasNext()) {
        Subscription subscription = cast(Subscription)subscrIt.next();
        subscrIt.remove();
        log.debug_(String.format("Removed subscription %s from service %s", new Object[] { subscription.getUuid(), service.getServiceId() }));
      }
    }
  }
}
