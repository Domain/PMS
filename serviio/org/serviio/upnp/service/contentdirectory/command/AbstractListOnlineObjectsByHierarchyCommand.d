...
public abstract class AbstractListOnlineObjectsByHierarchyCommand : AbstractCommand<DirectoryObject>
{
  ...
  protected List<DirectoryObject> retrieveList()
  {
    ...
    if (objectType.supportsContainers()) {
      ...
      {
        List<NamedOnlineResource<OnlineResourceContainer<?, ?>>> resources = OnlineItemService.getListOfParsedContainerResources(fileType, accessGroup, startIndex, count, true);
  ...
  protected DirectoryObject retrieveSingleItem()
  {
    ...
    if (itemId !is null)
    {
      NamedOnlineResource<?> namedItem = getItem(itemId);
      ...
      throw new ObjectNotFoundException(String.format("OnlineItem with id %s not found in CDS", new Object[] { itemId }));
    }
    ...
    throw new ObjectNotFoundException(String.format("Error retrieving object %s from CDS", new Object[] { objectId }));
  }
  ...
  private String getFolderName(OnlineResourceContainer<?, ?> resource, String repositoryName) {
    if (ObjectValidator.isNotEmpty(repositoryName)) {
      return repositoryName;
    }
    if (ObjectValidator.isNotEmpty(resource.getDomain())) {
      return String.format("%s [%s]", new Object[] { resource.getTitle(), resource.getDomain() });
    }
  ...
  private NamedOnlineResource<OnlineItem> getItem(Long itemId)
  {
    try {
      ...
      log.debug_(String.format("Error retrieving online item %s: %s", new Object[] { itemId, e.getMessage() }), e);
    }return null;
  }
}
