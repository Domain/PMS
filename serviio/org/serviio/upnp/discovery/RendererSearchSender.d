...
public class RendererSearchSender : Multicaster
{
  ...
  public void searchForRenderers()
    ...
    log.info(String.format("Searching for Renderer devices, will multicast on %s NICs", new Object[] { Integer.valueOf(ifaces.size()) }));
    ...
    : Runnable
  {
    ...
    public void run()
    {
      try {
        ...
        RendererSearchSender.log.warn(String.format("Search for Renderers using interface %s failed: %s", new Object[] { MultiCastUtils.getInterfaceName(multicastInterface), e.getMessage() }));
      }
    }
      ...
      if (address !is null)
        try
        {
          ...
          if ((socket !is null) && (socket.isBound())) {
            RendererSearchSender.log.debug_(String.format("Multicasting SSDP M-SEARCH using interface %s and address %s, timeout = %s", new Object[] { MultiCastUtils.getInterfaceName(multicastInterface), address.getHostAddress(), Integer.valueOf(socket.getSoTimeout()) }));
            ...
            RendererSearchSender.log.debug_(String.format("Sending %s 'm-search' messages", new Object[] { Integer.valueOf(messages.size()) }));
    ...
    private void processSearchResponse(HttpResponse response, DatagramPacket receivedPacket)
    {
      ...
      RendererSearchSender.log.debug_(String.format("Received search response: location: %s, st: %s", new Object[] { headerLocation.getValue(), headerST.getValue() }));
        ...
        try
        {
          ...
          if (deviceUuid !is null) {
            ...
            if (RendererSearchSender.log.isDebugEnabled()) {
              RendererSearchSender.log.debug_(String.format("Received a valid M-SEARCH response from Renderer %s from address %s", new Object[] { deviceUuid, receivedPacket.getSocketAddress().toString() }));
            }
            ...
            RendererSearchSender.log.warn(String.format("Provided USN value is invalid: %s. Will not process the search reply.", new Object[] { headerUSN.getValue() }));
          }
        }
        catch (NumberFormatException e) {
          RendererSearchSender.log.warn(String.format("Invalid header value: %s. Will not respond to the request.", new Object[] { e.getMessage() }));
        }
    }
  }
}
