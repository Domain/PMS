...
public class MetadataServerResource : AbstractServerResource
  : MetadataResource
{
  private static final Logger log = LoggerFactory.getLogger(MetadataServerResource.class);
  ...
  private List<ExtractorType> createNewVideoExtractors(MetadataRepresentation rep)
  {
    ...
    if (!descriptiveMDExtractor.equals("NONE")) {
      try {
        ...
        log.warn(String.format("Unrecognised extractor type '%s', using NONE", new Object[] { descriptiveMDExtractor }));
      }
    }
    return configs;
  }
